...
class GraphiquePlateau:
    ...
    def afficher_plateau_IA(self):
        ...
        self.canvas.bind("<Button-1>", self.gestion_clique)
    ...
    def gestion_clique(self,event):
        # Entraînement de l'IA
        ia = IA_DQN(self.plateau, alpha=0.001, gamma=0.9, epsilon=0.2 )
        nb_episodes = 2  # Nombre de parties d'entraînement

        for episode in range(nb_episodes):
            ia.plateau = Plateau()  # Réinitialiser le plateau à chaque partie
            state = ia.liste_plateau()  # Obtenir l'état initial du plateau
            game_over = False
            tour = 1  # Commence avec le joueur 1
            self.plateau = ia.plateau  # Mettre à jour le plateau graphique

            while not game_over:
                coups = ia.coup_possible(tour)  # Trouver les coups possibles
                
                if coups == []:  # Si aucun coup possible, la partie est terminée
                    game_over = True
                    reward = ia.r_defaite if tour == 1 else ia.r_victoire
                    ia.store_experience(state, None, reward, None)
                    print("partie terminée")
                
                else:
                    action = ia.choisir_action(state, coups)
                    ia.plateau.deplacementIA(action)  
                    
                    reward = 0
                    if action[2] == 1:
                        reward = ia.r_elimination
                    if ia.plateau.plateau[action[1]] is not None and ia.plateau.plateau[action[1]][1]:
                        reward = ia.r_dame
            ...
            print(f"Episode {episode+1}/{nb_episodes} terminé.")

    def afficher_plateau_IA_joueur(self):
        fenetre = Tk()
        fenetre.title("Jeu de Dames")
        fenetre.geometry("500x600")
        ...
        self.canvas.bind("<Button-1>", self.gestion_clic_IA_joueur)

       
        fenetre.mainloop()
    
    def gestion_clic_IA_joueur(self, event):
        x = event.x // 50 +1 
        y = event.y // 50 +1
        ...
        if self.turn == 0:
            ia = IA_DQN(self.plateau, alpha=0.001, gamma=0.9, epsilon=0 )       
            state = ia.liste_plateau()  # Obtenir l'état initial du plateau                    
            coups = ia.coup_possible(0)  # Trouver les coups possibles
                    
            if coups == []:  # Si aucun coup possible, la partie est terminée
                game_over = True
                reward = ia.r_defaite if 0 == 1 else ia.r_victoire
                ia.store_experience(state, None, reward, None)
                print("partie terminée")        
            else:
                action = ia.choisir_action(state, coups) 
...
window.afficher_plateau_IA_joueur()